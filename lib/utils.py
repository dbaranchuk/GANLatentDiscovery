...
from lib.metric.metric import get_fake_images_and_acts, compute_fid
from torchvision import utils
from lib.metric.inception import InceptionV3


def sample_noise(batch_size, code_size, mixing=False, device='cuda'):
    if mixing and random.random() < 0.9:
        gen_in11, gen_in12, gen_in21, gen_in22 = torch.randn(4, batch_size, code_size, device=device).chunk(4, 0)
        gen_in1 = [gen_in11.squeeze(0), gen_in12.squeeze(0)]
        gen_in2 = [gen_in21.squeeze(0), gen_in22.squeeze(0)]

    else:
        gen_in1, gen_in2 = torch.randn(2, batch_size, code_size, device=device).chunk(2, 0)
...
def sample_data(dataset, batch_size, image_size=256):
...
def evaluate(inception, G_running, iteration, sample_num=5000):
    """Custom GAN evaluation function"""

    # save images
    gen_i, gen_j = gen_sample.get(256, (10, 5))

    images = []
    with torch.no_grad():
        for i in range(gen_i):
            images.append(G_running([fixed_noise[i].cuda()])[0].cpu())

    sample_path = f'samples/{dataset_name}/{str(iteration).zfill(6)}.png'
    utils.save_image(torch.cat(images, dim=0), sample_path, nrow=gen_i, normalize=True, range=(-1, 1))

    # compute evaluation metrics
    fake_images, fake_acts = get_fake_images_and_acts(inception, G_running, code_size,
...
# TODO: check whether can be replaced by model.eval() -> model.train()
